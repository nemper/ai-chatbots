...
from myfunc.various_tools import play_audio_from_stream, transcribe_audio_file
...
def main():
    ...
    with st_fixed_container(mode="fixed", position="top", border=False): # snima audio za pitanje
            audio = audiorecorder("⏺ Pitaj", "⏹ Završi pitanje") # mozda ce biti zamenjeno ka 4o bude razumeo audio
            if len(audio) > 0:
    ...
    if prompt:
        ...
        if st.session_state.pricaj and full_response !="":
            ...
            audio_html = f"""
                        ...
                        <button id="playButton" onclick="document.getElementById('audio').play()">🔈 Odgovor</button>
                        <style>
                            #playButton {{
